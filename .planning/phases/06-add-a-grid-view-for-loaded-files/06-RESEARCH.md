...
- SwiftData: treat persistence as an explicit non-goal for this phase. The project already uses SwiftData for recent coordinates and batch history, but table/grid mode, thumbnails, selection, and file review state must remain in-memory and must not introduce `@Query`, new `@Model` types, `ModelContext` writes, or schema changes. [CITED: `.agents/skills/swiftdata-pro/SKILL.md`]
